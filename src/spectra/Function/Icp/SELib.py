...
def cal_SE_with_Pg_Te_Ne_single_Atom_(
    atom: _Atom.Atom,
    ...
    if atmos.Ne < 1.0:
        raise ValueError("must provide Ne > 0")
...
def make_ion_pop_(atom: _Atom.Atom, stage_pop: T_ARRAY | None):
    ...
    if stage_pop.ndim != 1:
        raise ValueError("stage_pop must be a 1 dimensional array")
    stages = atom.Level["stage"]
    nstage = len(set(stages.tolist()))
    if stage_pop.size != nstage:
        raise ValueError(
            f"stage_pop has size={stage_pop.size}, conflicts with the given atomic model has number of ionization stage = {nstage}"
        )
