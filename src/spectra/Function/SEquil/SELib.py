...
##        atmos.Nh  = atmos.Ne / ( 1.E-4 + Nh_SE[-1] )          # [cm^{-3}]
    ...
    if is_hydrogen:
        if is_single_element:
            raise ValueError(f"Please keep is_single_element=False when is_hydrogen=True")
...
def _B_Jbar_(Line : T_ARRAY, Line_mesh_Coe : T_ARRAY,
    ...
    for k in range(nLine):
        ...
        if Line['f0'][k] <= 0:
            absorb_prof_cm_all[i_start:i_end] = 0.
...
def _solve_SE_(nLevel : T_INT, idxI : T_ARRAY, idxJ : T_ARRAY, 
               ...
               rate_only:T_BOOL) -> T_TUPLE[T_ARRAY,T_ARRAY,T_ARRAY] :
