...
def read_table_(file: str):
    ...
    with Path(file).open() as f:
        ...
        for line in f:
            ...
            #     'ev' : ev,
            # }
            datas[idx] = LevelRecord(conf=conf, term=term, j=J, g=g, stage=stage, ev=ev)
            # count += 1
    return datas, prefix
...
def read_rh_level_(file: str):  # noqa: C901
    ...
    def is_start_(text: str):
        if not text.startswith("#"):
            return False
        if "Energy levels" in text:
            return True
        return False

    def is_end_(text: str):
        if text.startswith("#") and "qcore" in text:
            return True
        return False
...
def make_Level_file_(
    ...
    with Path(outfile).open("w") as f:
        ...
        for _k, rec in tables.items():
            ...
            values = (rec.conf, rec.term, rec.j, n, L, s2p1, rec.g, rec.stage, rec.ev)
            text = " " * 4
            for v in values:
                if isinstance(v, str):
                    text += f"{v:<12s}"
...
def read_rh_line_(file: str):  # noqa: C901
    ...
    def is_end_(text: str):
        if text.startswith("#") and "alpha [m^-2]" in text:
            return True
        return False
    ...
    with Path(file).open() as f:
        ...
        for line in f:
            ...
            linerh = LineRH(
                ...
                qwing=qwing,
                vdWapprx=vdWapprx,
                vdWaals_H_1=vdWaals_H_1,
                vdWaals_H_2=vdWaals_H_2,
                vdWaals_He_1=vdWaals_He_1,
                vdWaals_He_2=vdWaals_He_2,
                radiative=radiative,
                stark=stark,
            )
            datas.append(linerh)
...
def make_Aji_file_(lines: list[LineRH], tables: dict[int, LevelRecord], outfile: str, prefix: str | None):
    with Path(outfile).open("w") as f:
        ...
        text = "#" + " " * 3
        names = ("conf_i", "term_i", "J_i", "conf_j", "term_j", "J_j", "Aji[s^-1]", "Wave[AA]")
        for name in names:
            text += f"{name:<12s}"
        f.write(text + "\n")
...
def read_rh_alpha_(file: str):  # noqa: C901
    ...
    def is_start_(text: str):
        if not text.startswith("#"):
            return False
        if "alpha [m^-2]" in text:
            return True
        return False

    def is_end_(text: str):
        if not text.startswith("#") and "TEMP" in text:
            return True
        return False
    ...
    with Path(file).open() as f:
        ...
        for line in f:
            ...
            if not isblock and len(words) == nwords:
                j = int(words[0])
                i = int(words[1])
                alpha0 = float(words[2]) * 1.0e4  # [m^2] --> [cm^2]
                nlambda = int(words[3])
                wavedep = words[4]
            ...
            if not isblock:
                continue
            assert len(words) == 2, "text splited by whitespace must have length==2"
            wave = float(words[0])  # [nm]
            alpha = float(words[1]) * 1.0e4  # [m^2] --> [cm^2]
...
#-----------------------------------------------------------------------------
    ...
    with Path(outfile).open("w") as f:
        ...
        text = "#" + " " * 3
        names = ("conf_i", "term_i", "J_i", "conf_j", "term_j", "J_j", "nLambda", "Wave Depen", "alpha0[cm^2]")
        for name in names:
            text += f"{name:<12s}"
        f.write(text + "\n")
        f.write(COMMENT_ + "\n")
        text = "#" + " " * 3
        text += "wavelength[nm]    cross section [cm^2]"
        f.write(text + "\n")
        f.write(COMMENT_ + "\n")
...
def read_rh_collision_(file: str):  # noqa: C901
    ...
    def is_end_(text: str):
        if not text.startswith("#") and "END" in text:
            return True
        return False
        ...
        for line in f:
            ...
            if not isstart:
                if is_start_(line):
                    ...
                    words = [w.strip() for w in line.split()]
                    ntemp = int(words[1])
                    rhtemp = ColTempRH(ntemp=ntemp, temps=[float(w) for w in words[2 : 2 + ntemp]])
                continue
            ##: check whether to end reading
            if is_end_(line):
                break
...
def main():
    ...
    parser.add_argument("rhpath", metavar="rhpath", type=str, help="path of rh *.atom file")
    parser.add_argument(
        "conftable", metavar="conftable", type=str, help="path of RH level index --> *.configuration-table.txt"
    )
    parser.add_argument("symbol", metavar="symbol", type=str, help="atomic symbol of the element")
    parser.add_argument(
        "--outdir", metavar="DIR", type=str, default="./", help="directory to output configuration files"
    )
    parser.add_argument(
        "--title", metavar="title", type=str, default="please modify this title line", help="title information"
    )
    parser.add_argument(
        "--use-rh-energy",
        action="store_true",
    ...
    parser.add_argument(
        "--file-prefix", metavar="fprefix", type=str, default="tmp.", help="prefix of generated configuration files"
    )
    parser.add_argument(
        "--indir",
        metavar="indir",
