...
def read_general_info_(rs: T_INT, lns: T_LIST[T_STR]) -> T_TUPLE[T_INT, T_STR, T_INT, T_STR, T_INT]:
...
def read_PI_table_(  # noqa: C901
    ...
    PI_wdep_dict: T_DICT[T_INT, T_STR],
    PI_coe: T_ARRAY,
    level_info_table: T_CTJ_TABLE,
    cont_ctj_table: T_CTJ_PAIR_TABLE,
):
...
def make_Atom_Level_(
    ...
    for k in range(nLevel):
        _Level_info_table.append((Level_info["configuration"][k], Level_info["term"][k], Level_info["J"][k]))
...
def make_Atom_Line_(
    ...
    Line_ctj_table: T_CTJ_PAIR_TABLE,
    atom_type: T_E_ATOM,
) -> T_TUPLE[T_ARRAY, T_E_ATOMIC_DATA_SOURCE]:
    ...
    for k in range(nLine):
        if Line["AJI"][k] <= 0.0:
            Line["BJI"][k], Line["BIJ"][k] = 0.0, 0.0
            continue
        Line["BJI"][k], Line["BIJ"][k] = _LTELib.einsteinA_to_einsteinBs_cm_(
            Line["AJI"][k], Line["w0"][k], Line["gi"][k], Line["gj"][k]
        )
...
def make_Atom_CECI_(
    ...
    Level: T_ARRAY,
    Level_info_table: T_CTJ_TABLE,
    Tran_ctj_table: T_CTJ_PAIR_TABLE,
) -> T_TUPLE[
    T_ARRAY,
    T_ARRAY,
    T_ARRAY,
    ...
    if path is None:
        ...
        for i, key in enumerate(sorted_keys):
            ...
            if PI_wdep_dict[key] == "Hydrogenic":
                ...
                z = Level["stage"][Coe["idxI"][i]]  # type: ignore[assignment]
                ni = Level["n"][Coe["idxI"][i]]
                arr2d[1, :] = _DegenerateN.compute_Hydrogenic_PI_cross_section_(z, ni, Eionize, Cont_mesh[i, :])  # type: ignore[arg-type]
            else:
                arr2d = alpha_table_dict[key]
                alpha_table_idxs[i, 1] = alpha_table_idxs[i, 0] + arr2d.shape[1]
                bias = alpha_table_idxs[i, 1]

                arr2d[0, :] *= 1.0e-7  # unit conversion of wavelength, nm --> cm   (cm, cm^2)
