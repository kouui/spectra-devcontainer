...
def Level_ctj_table_to_Level_info_(
    Level_ctj_table: T_LIST[T_TUPLE[T_STR, T_STR, T_STR]],
) -> T_DICT[T_STR, T_LIST[T_STR]]:
...
def line_list(atom: _Atom.Atom, Aji_min: T_FLOAT = 1e4):
    ...
    print("id   wavelength[A]  Aji[s-1]   lower level ctj    -     upper level ctj ")
    for sel in selects:
        id = sel["id"]
        wv0 = sel["w0_AA"]
        Aji = sel["Aji"]
