...
from ..Util.AtomUtils import AtomInfo, AtomIO
...
def line_with_text_(
    _ax, _lp, _rp, _text, _tsize, _r, _tangle=0, _lcolor="black", _lwidth=0.7, _lstyle="-", _tcolor="black"
):
    r"""
    Parameters
    ----------
...
class Grotrian:
    def __init__(self, _atom, _path=None, _conf_prefix=None, _scaleFunc=None, _scaleFunc_inv=None):
        ...
        self.set_atom(
            _atom, _path=_path, _conf_prefix=_conf_prefix, _scaleFunc=_scaleFunc, _scaleFunc_inv=_scaleFunc_inv
        )
    ...
    def set_atom(self, _atom, _path, _conf_prefix=None, _scaleFunc=None, _scaleFunc_inv=None):
        ...
        # ---------------------------------------------------------------------
        singlet, multiplet, Lset, self.group_dict = _prepare_dict_(
            _atom=_atom, _conf_prefix=self.prefix, _scaleFunc=_scaleFunc, _exclude=_exclude
        )
    ...
    def make_fig(self, _fig=None, _axe=None, _figsize=(6, 8), _dpi=120, _f=200, _removeSpline=None, _resetFig=True):  # noqa: C901
        ...
        for _text_tuple, _val in self.group_dict.items():
            ...
            plt.text(
                x_text,
                y_text,
                f"{_text_tuple[0]} {_filter_term(_text_tuple[1])}",
                fontsize=_textsize,
                color="k",
            )
        ...
        for k0, v0 in singlet.items():
            for k1, v1 in v0.items():
                ...
                x_text = xs_level[1] + _st
                y_text = ys_level[0]
                plt.text(x_text, y_text, f"{k0[0]} {_filter_term(k0[1])}", fontsize=_textsize, color="k")
        ...
        for k0, v0 in multiplet.items():
            ...
            plt.text(x_pos_text, y_mean, f"{k0[0]} {_filter_term(k0[1])}", fontsize=_textsize, color="k")
        # ---------------------------------------------------------------------
        ...
        if self._scaleFunc is not None and self._scaleFunc_inv is not None:
            ...
            ax.set_yticklabels(
                [f"{self.atom.Level['erg'][:].max() / CST.eV2erg_ * self._scaleFunc_inv(tick):.2f}" for tick in yticks]
            )
        # ---------------------------------------------------------------------
            ...
            if _text_selection == "all":
                ...
                if wl != "None":
                    if not _hasText:
                        wl = "None"
                    self.connect_line(
                        _cfj1=_ctj1, _cfj2=_ctj2, _r1=_r1, _r2=_r2, _c="black", _text=wl, _tsize=7, _r=0.4
                    )
    ...
    def plot_transition_rate(
        ...
        _asize=5,
        _lwidth=2,
        _level_ctj_without_prefix=None,
    ):
        r""" """
        ...
        for _ctj_ij_plot in ctj_rate_dict:
            # print(_ctj_ij_plot)

            ## single level related
            if _level_ctj_without_prefix is not None:
                _level_ctj = (
                    self.prefix + _level_ctj_without_prefix[0],
                    _level_ctj_without_prefix[1],
                    _level_ctj_without_prefix[2],
                )
            ...
            _annotation_obj = self.connect_arrow(
                _ctj_i, _ctj_j, _ri, _rj, _direction, _cmap, _norm, _v, _abserr=_abserr, _asize=_asize, _lwidth=_lwidth
            )
