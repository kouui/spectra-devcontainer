...
def set_imshow_ticks_(axe, arr, axis, points=None, fmt="%1.3f", rot=0, fontsize=None):
    ...
    if axis not in ("x", "y"):
        raise ValueError("`axis` argument should either be 'x' or 'y'")
...
def transition_heatmap0(
    atom, SE_con, Rate_con, vmin=None, vmax=None, klevel_max=None, cmap="cool", title_prefix="", figsize=(9, 4)
):
    # SE_con = SE_He
    # Rate_con = Rate_He
    numeric_error = 1e-17
    if klevel_max is None:
        klevel_max = atom.nLevel
    # klevel_max = 21
    ...
    if vmin is None:
        vmin = min([mat.min() for mat in tran_mat.values()])
    if vmax is None:
        vmax = max([mat.max() for mat in tran_mat.values()])
