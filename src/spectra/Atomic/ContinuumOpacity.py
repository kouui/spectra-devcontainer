...
def HI_bf_LTE_cross_sec_(Te: T_VEC_IFA, w: T_VEC_IFA) -> T_VEC_FA:
    ...
    if n_limit > 7:
        ...
        for n in range(n_limit, min(n_limit + 3, 7) + 1):
            ...
            alpha += n * n * _exp(-1.0 * fac) * hydrogenic_bf_cross_sec_n_(n, w, 1) * 2.0

        ## Cst.h_ * Cst.c_ / Cst.k ~ 1.438787
        ## correction factor
        alpha *= (1.0 - _exp(-1.0 * CST.h_ * CST.c_ / w / kT)) / u
...
def Hp_ff_cross_sec_(Te: T_VEC_FA, w: T_VEC_FA, Ne: T_VEC_FA, Z: T_INT = 1) -> T_VEC_FA:
    ...
    sgm = 3.6927e8 * Ne * Z**2 / _sqrt(Te) / (CST.c_ / w) ** 3 * gff  # Rutten

    u = 2.0
    a0 = (1.0 - _exp(-1.0 * CST.h_ * CST.c_ / w / kT)) / u
    alpha = a0 * sgm
...
def HI_ff_cross_sec_(Te: T_VEC_IFA, w: T_VEC_IFA) -> T_VEC_FA:
    ...
    alpha = 0.66699 * Te**2.5 * _exp(-157779.0 / Te) * ahiff

    alpha *= 0.5 * (1.0 - _exp(-1.0 * CST.h_ * CST.c_ / w / kT))

    return alpha * 1.0e-26
...
def H_LTE_continuum_opacity_(Te: T_VEC_FA, Ne: T_VEC_FA, Nh: T_VEC_FA, w: T_VEC_FA) -> T_VEC_FA:
    ...
    """
    # /*  -----   solving LTE Saha's eq. for hydrogen   -----	*/
    q = 2.0 * 2.07e-16 * Te ** (-1.5) * 10.0 ** (5040.0 * 13.6 / Te) * Ne
    N_HI = q / (q + 1.0) * Nh
    # Np   = 1. / ( q + 1. ) * Nh
    Np = Nh - N_HI
    kappa = N_HI * (
