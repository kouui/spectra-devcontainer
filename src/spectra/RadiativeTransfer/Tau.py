...
def make_tau_(ND: T_INT, e_max: T_FLOAT, relax: T_FLOAT = 0.0):
    """
    ...
    mid = e_max - _numpy.log10(2.0)
    tau[: ND // 2 + 1] = (
        _numpy.logspace(e_min, mid, ND // 2 + 1) * (1 - relax)
        + _numpy.linspace(10.0**e_min, 10.0**mid, ND // 2 + 1) * relax
    )
