...
class TestGauntFactorCoe:
    ...
    def test_coe(self, ref, ni, i):
        assert_close(Hydrogen.gaunt_factor_coe_(i, ni), ref[f"Hydrogen.gaunt_coe_i{i}_ni{ni}"])


class TestPICrossSectionX:
    def test_above_threshold(self, ref):
        assert_close(Hydrogen.PI_cross_section_(1, 2.0, 1), ref["Hydrogen.PI_x_ni1_x2_Z1"])

    def test_ni2(self, ref):
        assert_close(Hydrogen.PI_cross_section_(2, 1.5, 1), ref["Hydrogen.PI_x_ni2_x1p5_Z1"])

    def test_below_threshold(self, ref):
        assert_close(Hydrogen.PI_cross_section_(1, 0.5, 1), ref["Hydrogen.PI_x_ni1_x0p5_Z1"])
