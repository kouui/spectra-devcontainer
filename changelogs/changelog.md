...
### `tests/unittest/test.MALI_GlobalMesh.py` — YW.Huang

complete the global-mesh verification for the transition-complete axis: mixed line + continuum merges (a continuum window swallows the interleaved line windows while every input point stays locatable in its own window), an exactly-shared point deduplicates across mesh types into one axis point that both windows locate, and mixed-scale meshes keep near-but-distinct points separate (the dedup tolerance derives from the finest intra-mesh spacing, so a coarse continuum point can never be absorbed by a nearby line point).

### `notebooks/demo/mali_global_mesh/{build_notebook.py, mali_global_mesh.ipynb}` — YW.Huang

new generated demo running the global-mesh machinery on theoretical hydrogen (`init_theoretical_hydrogen_atom_(10)`: 36 lines, 9 continua, no data files) to measure the design decisions of the dual-mesh architecture (`reference/RH_wavelength_mesh_explained.txt`). Overlap onset: window overlap is a wing-extent property — Doppler-sized wings (qwing 10) keep all 36 hydrogen windows disjoint, damping-sized wings (qwing 300) chain the Lyman series, and `span` grows past `nLambda` (41 → 155) because a window must include every interleaved foreign point; dedup fires on exact duplicates only, which lines alone never produce — the axis is built **transition-complete** (36 lines + 9 continuum meshes merged together; `merge_meshes_` is transition-agnostic, and the toy loop's passive continua are a consumer's simplification, never the builder's), which is what makes dedup fire (8 shared points) and hands a continuum window 806 interleaved points. The SE quadrature is measured in both regimes of the dual-mesh design: static — direct window quadrature and the interpolate-`J`-to-local-mesh route coincide (~5e-8, dense grids appearing only as error oracles, never as either route's mesh; the reference's validation test 1), with `wphi` renormalization as the accuracy lever (5e-2 → 3e-6 on an 11-point window); shifted — measured against the real quiet-Sun atlas (no synthetic `J` shape) at H alpha (11.2 km/s, the ruler-owner) and Ca II K (1.6 km/s, a narrow victim on the shared 10 km/s axis), `V_los` = 0..300 km/s in 10 km/s steps, for a static and a co-moving background (validation test 3). Findings: the dominant obligations are build-time axis properties binding both routes — extent must cover the excursion (beyond it both fail identically at 0.2–0.7) and density over the excursion band must resolve `J`'s real feature scale (the co-moving case — `J`'s own core riding with `V_los` onto the coarse wings — floors both routes equally at ~2e-2 for Ca II K on 10 km/s spacing; only 0.8 km/s spacing reaches ~2e-4; no interpolation and no `wphi` recovers structure the axis never sampled); the dual-mesh route contributes phi-side exactness — up to ~200 times more accurate than direct on the narrow line at moderate shifts (V = 30–50 km/s) and bounded where direct loses profile support entirely (0/0 → nan at V ≥ 150) — but cannot lower the `J`-structure floor. A V = 0 anatomy (values-vs-weights decomposition on Ca II K) locates the dual route's static penalty on a narrow shared-ruler line in the one-signed chord bias of the single global → local interpolation (the phi-weighted bias equals the whole error; oracle-J-at-local-nodes at ~1e-5 clears node placement and weights), while direct — exact values under wphi-renormalized symmetric weights — is exact for locally linear `J`; the penalty peaks at V = 0 generically because phi then sits on the curvature its own transition carves into `J`. A companion H alpha V = 100 anatomy pictures the extent-violation mode: past the window's last node the axis has no `J`, direct truncates the unsupported half of phi and dual clamps `J` to the edge value — both one-sided and wrong by O(0.2), unrepairable by any quadrature choice. The committed static MALI toys are unaffected (the regimes coincide at `V_los = 0`); aligning the toy loop with the dual-mesh SE quadrature is tracked separately. Interpolation at the pipeline edges, both measured: cross-section tables onto the global axis before the loop (linear interpolation leaves 9e-2 error in the coarse short-wavelength tail where alpha spans decades; log-log interpolation flattens it to 1e-3 — a full-MALI recommendation) and emergent intensity onto an observational mesh after convergence (quadratic in window resolution, never feeds back).
